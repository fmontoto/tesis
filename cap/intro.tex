...
A wager can be placed on many different events, in a casino we find randomizing
  ...
  racing, football games, etc. or the minimum temperature in Santiago during a
  particular night.
Gambling popularity and the large amounts of money at stake inevitably entails a
  lot of interest in such activities.
Most of the time gambling is heavily regulated and taxed, and lotteries are
...
This and the massive internet use has moved the gambling industry online%
  \cite{shelat2002makes}\cite{griffiths2008internet}.
...
Even if the bet takes place in a physical casino, where the law can enforce
  the bet, it is not certain if the casino will be able to pay after the
resolution.
...
Friends usually are trusted people, so trusting them when gambling might not
  ...
  friend, who helds the money and pass it to the winner when the bet is
  resolved.
Online casinos on the other hand are more problematic as there are many knowns
...
This kind of characteristics could help to indicate thar online site is
  trustworthy.

But, what if you would like to gamble in a event that no gambling site offers
  nor any friend is willing to do gamble init? Likely the internet would be the
...
(More on online casinos at subsection \ref{gambling}.)
...
\begin{enumerate}
	\item Provide a protocol to make it possible to gamble with untrusted peers over
	  real world events.
	\item Provide the correct economic incentives to the protocol participants to
	  behave correctly, so everyone's incentives are aligned.
	\item Implement a proof of concept of the designed protocol.
	\item Debate implications and other applications for the designed protocol.
\end{enumerate}
...
If enough oracles do it, the bet can take place.
...
Oracles get their payment by answering who the winner is, and as soon as
  ...
  \frac{n}{2} \rfloor + 1$ with $n$ the number of oracles.} oracles vote for
  one player as the winner, this player can take its winnings.
