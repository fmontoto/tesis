...
\transaction
    ...
    {$\mathcal{P} + n/2 \cdot $($[Registration] \\
        ...
        {(\signature{A} $\wedge$ ($AWins_{\tilde{o}}$ $\wedge$ ...\tikzmark{ini:f})) \\
                          \vee \\
         (\signature{B} $\wedge$ ($BWins_{\tilde{o}}$ $\wedge$ ...\tikzmark{ini:g})) \\
                                    \vee \\
         (\signature{A} $\webge$ \timeout{two})}
    {$\mathcal{P} + n/2 \cdot $($[Registration] \\
            - [Oracle Payment] \\
            - \fee{Oracle Inscription}$)$ \\
            - 1/2 $($\fee{Bet Promise} + \fee{Bet}$)}
        {(\signature{A} $\wedge$ ($AWins_{\tilde{o}}$ $\wedge$ ...\tikzmark{ini:ff})) \\
                          \vee \\
         (\signature{B} $\wedge$ ($BWins_{\tilde{o}}$ $\wedge$ ...\tikzmark{ini:gg})) \\
                                    \vee \\
         (\signature{B} $\webge$ \timeout{two})}
...
There are n + 2 inputs to this transaction, the first two brings the money
  corresponding to the winner's prize, its come for the commitment of
  bot players at the \textit{Bet Promise} transaction.
The next n come from each oracle inscription and require the signature of both
  players and the corresponding oracle.
